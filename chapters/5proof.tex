...
\begin{thm}\label{thm:two_sided_cobar_D}
	...
	\begin{proof}
		Denote $\Cobar(\cdots)$ by $\Cobar$. Singular chains are free abelian groups, hence flat over $\Z$. Moreover, over $\Z$, submodules of free modules are free. Lastly, the associated gradeds of the singular chains are free. To see this, notice that the coradical filtration on singular chains is just the skeletal filtration:
		\[F_rC_\bullet(X) = \bigoplus_{k=0}^r C_k(X),\]
		so that the associated gradeds are just the levelwise singular chain groups, which are free.
		
		Therefore, by Lemma \ref{lem:graded_quism} and Lemma \ref{lem:normalized_inclusion_is_graded_quism}, it suffices to exhibit a quasi-isomorphism from 
...
\begin{proof}[Proof of Theorem \ref{thm:looijenga}]
    ...
    whose kernel if isomorphic to $\Z$ is $a=b$ and trivial otherwise. It remains to see that if $a=b$, the kernel is generate by the constant path at $a$.
\end{proof}
