...
to obtain a new bicomplex, which we will call $\mathscr{D}^\bullet_\bullet$. It follows from Corollary \ref{cor:topological_aw} and Proposition \ref{lem:quasiiso_bicomplex} that this map $\mathscr{C} \to \mathscr{D}$ of bicomplexes is a quasi-isomorphism. By definition of tensor product of chain complexes, $\mathscr{D}$ has a bigrading given by
\begin{equation}
\mathscr{D}^q_p = (C_\bullet(X)^{\tensor q})_p = \bigoplus_{n_1 + \cdots + n_q = p} C_{n_1}(X) \tensor \cdots \tensor C_{n_q}(X).
\end{equation}

\begin{prop}\label{prop:two_sided_cobar_D}
	There is a natural isomorphism $\Cobar(C_\bullet(\{b\}), C_\bullet(X), C_\bullet(\{a\})) \to \mathscr{D}$.
	\begin{proof}
		Denote $\Cobar(C_\bullet(\{b\}), C_\bullet(X), C_\bullet(\{a\}))$ by just $\Cobar$. We have  $C_\bullet(\{a\}) \cong \Z \cong C_\bullet(\{b\})$, hence natural isomorphisms 
		\[C_\bullet(\{a\}) \tensor C_{b_1}(X) \to C_{b_1}(X)\]
		\[C_{b_q}(X) \tensor C_\bullet(\{b\}) \to C_{b_q}(X)\]
		yielding a natural isomorphism
		\begin{equation}
			\begin{split}
			\phi: \Cobar^q_p &=  \bigoplus_{n + b_1 + \cdots + b_q + m = p} C_n(\{a\}) \tensor C_{b_1}(X) \tensor \cdots \tensor C_{b_q}(X)\tensor C_m(\{b\}) \\
			&\to \bigoplus_{b_1 + \cdots + b_q = p}  C_{b_1}(X) \tensor \cdots \tensor C_{b_q}(X) = \mathscr{D}^q_p.
			\end{split}
		\end{equation}
		We now check that the differentials are compatible with $\phi$. Because $C_\bullet(\{a\})$ and $C_\bullet(\{b\})$ are chain complexes where all differentials are zero, the horizontal cobar differential formula \ref{eqn: twoside_cobar_differential} gives 
		\begin{equation}
		\begin{split}
			\phi(d_H^{\Cobar}(m \tensor c_1\tensor \cdots \tensor c_1\tensor n)) &= \phi(\sum_{i=1}^q (-1)^{\sigma(c_i)} \partial_M(m) \tensor c_1 \tensor \cdots \tensor \partial_C(c_i) \tensor \cdots \tensor c_q \tensor \partial_N(n)) \\
			&= \sum_{i=1}^q (-1)^{\sigma(c_i)}  c_1 \tensor \cdots \tensor \partial_C(c_i) \tensor \cdots \tensor c_q  \\
	...
	\end{proof}
\end{prop}
The following theorem thus follows from our discussion of the cobar construction in Chapter 3, and the bicomplex quasi-isomorphisms constructed in this section:

\begin{thm}\label{thm:new_cobar}
    There is a quasi-isomorphism of bicomplexes
	\[\Cobar(C_\bullet(\{b\}), C_\bullet(X), C_\bullet(\{a\})) \to \mathscr{N}.\]
    \begin{proof}
    Composing the quasi-isomorphisms and equalities obtained so far, we have 
		...
		\[\begin{tikzcd}[column sep=3em]
			{\Cobar(C_\bullet(\{b\}), C_\bullet(X), C_\bullet(\{a\}))} & {\mathscr{D}} & {\mathscr{C}} & {\mathscr{N}}
			\arrow["\ref{prop:two_sided_cobar_D}", from=1-1, to=1-2]
