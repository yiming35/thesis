...
\begin{thm}\label{thm:two_sided_cobar_D}
	...
	\begin{proof}
		Denote $\Cobar(\cdots)$ by $\Cobar$. Because all modules involved are free, hence flat over $\Z$, it suffices by Lemma \ref{lem:graded_quism} to exhibit a quasi-isomorphism from 
...
\begin{proof}[Proof of Theorem \ref{thm:looijenga}]
    ...
    whose kernel if $\Z$ is $a=b$ and trivial otherwise.
\end{proof}