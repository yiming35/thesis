...
Each column $p$ of this bicomplex forms a cochain complex $\mathscr{C}_p^\bullet$ which is the Moore complex of $C_p(F^\bullet)$. Consider now the bicomplex $\mathscr{N}^\bullet_\bullet$ whose columns are instead the normalized cochain complexes of $C_p(F^\bullet)$. This is a bicomplex for the same reason as above, and the inclusion $\mathscr{N} \hookrightarrow \mathscr{C}$ is a quasi-isomorphism of bicomplexes by Dold--Kan and Lemma \ref{lem:quasiiso_bicomplex}. Let us spell out what the groups in $\mathscr{N}$ actually are. Following the definition, we have 
...
\begin{thm}\label{thm:two_sided_cobar_D}
	...
	and likewise for $C_\bullet(\{b\})$ as a left $C_\bullet(X)$-comodule. Then there is a natural quasi-isomorphism 
	\[\Cobar(NC_\bullet(\{b\}), NC_\bullet(X), NC_\bullet(\{a\})) \to \mathscr{D}.\]
	\begin{proof}
		Denote $\Cobar(\cdots)$ by $\Cobar$. We will exhibit an isomorphism from $\Cobar$ into a bicomplex that is quasi-isomorphic to $\mathscr{D}$. 
		
		Note that $NC_\bullet(\{a\})$ and $NC_\bullet(\{b\})$ are just the chain complexes with a $\Z$ term concentrated in degree zero, i.e. the identity object in the symmetric monoidal structure on $\catname{Ch}_+(\catname{Ab})$. Hence we have natural isomorphisms 
		...
		\begin{equation}
			...
			&\to \bigoplus_{b_1 + \cdots + b_q = p}  NC_{b_1}(X) \tensor \cdots \tensor NC_{b_q}(X) =: N\mathscr{D}^q_p.
			\end{split}
		\end{equation}
		where $N\mathscr{D}$ bicomplex obtained by taking normalized chains on each row of $\mathscr{C}^\bullet_\bullet$ then applying the Alexander--Whitney map, as in the diagram below:
		...
		\[\begin{tikzcd}[cramped]
			...
			{NC_\bullet(X^q)} & {NC_\bullet(X)^{\tensor q} =:N\mathscr{D}^q_\bullet}
			\arrow["{\AW_q}", from=1-1, to=1-2]
			\arrow[hook, from=2-1, to=1-1]
			\arrow["{\AW_q}"', from=2-1, to=2-2]
			\arrow[hook, from=2-2, to=1-2]
		\end{tikzcd}\]
		By Lemma \ref{lem:quasiiso_bicomplex}, because the Alexander--Whitney map is a graded quasi-isomorphism, the inclusion $N\mathscr{D}\hookrightarrow \mathscr{D}$ is a bicomplex quasi-isomorphism.

		We now check that the differentials on $\Cobar$ and $N\mathscr{D}$ are compatible with $\phi$. Because $NC_\bullet(\{a\})$ and $NC_\bullet(\{b\})$ have all differentials zero, the horizontal cobar differential formula \ref{eqn: twoside_cobar_differential} gives 
		\begin{equation*}
		\begin{split}
...
\end{split}
\end{equation*}
		The proof for $d^1, ..., d^q$ are analogous. Hence $\phi$ is a bicomplex quasi-isomorphism $\Cobar \to N\mathscr{D}$, and postcomposing with the inclusion $N\mathscr{D}\hookrightarrow \mathscr{D}$ yields the desired quasi-isomorphism.
...
\begin{proof}[Proof of Theorem \ref{thm:looijenga}]
	...
	The filtration on $H_0\tot\Cobar_{\leq n}$ has only two nonzero pieces, which by (stallings) are given by ... 
	% \[F_{-n}H_0\tot \cong E_{-n,n}^\infty \cong \mathcal{I}^n/\mathcal{I}^{n+1}, \quad F_0H_0\tot \cong E_{-0,0} = \mathcal{I}^0/ \mathcal{I} = \Z\pi_1(X,x)/\mathcal{I} \cong \Z\]
	% so we have an exact sequence 
	% \[0 \to \mathcal{I}^n/\mathcal{I}^{n+1} \to H_0\tot\Cobar_{\leq n} \to \Z\to 0\]
	yielding an isomorphism 
	\[H_0\tot\Cobar_{\leq n} \cong \Z\pi_X(a,b)/\Z\pi_X(a,b)\mathcal{I}^{n+1}.\]
    ...
    whose kernel if $\Z$ is $a=b$ and trivial otherwise, completing the proof.
\end{proof}