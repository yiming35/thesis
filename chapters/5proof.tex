...
\begin{prop}
    ...
    \begin{proof}
        ...
        At this point, because the only nonzero rows are $q=n$ and possibly $q=0$, the differentials on the pages $E^{\geq 2}$ are all zero, except possibly on page $E^{n+1}$, where we have a differential of bidegree $(-n-1, -n)$ from $E^{n+1}_{n+1, n} = H_{n+1}(X^n, X(n)^a_b)$ to $E^{n+1}_{0,0} = \Z$. 
    \end{proof}
\end{prop}
...
to obtain a new bicomplex, which we will call $\mathscr{D}^\bullet_\bullet$. It follows from Corollary \ref{cor:topological_aw} that this map $\mathscr{C} \to \mathscr{D}$ of bicomplexes is a quasi-isomorphism. By definition of tensor product of chain complexes, $\mathscr{D}$ has a bigrading given by
\begin{equation}
\mathscr{D}^q_p = (C_\bullet(X)^{\tensor q})_p = \bigoplus_{n_1 + \cdots + n_q = p} C_{n_1}(X) \tensor \cdots \tensor C_{n_q}(X).
\end{equation}
\begin{prop} Suppose $x:= a=b$. The coface maps $d^i: X^q \to X^{q+1}$ and codegeneracy maps $s^i: X^q\to X^{q-1}$ respectively induce maps $\mathscr{D}(d^i): \mathscr{D}^q_\bullet \to \mathscr{D}^{q+1}_\bullet, \quad \mathscr{D}(s^i): \mathscr{D}^q_\bullet \to \mathscr{D}^{q-1}_\bullet$ given by
	\[\mathscr{D}(d^i) = \begin{cases}
		\eta(1)\tensor \id^{\tensor q} & i=0 \\
		\id^{\tensor i-1}\tensor \Delta \tensor \id^{\tensor q-i} & i \in \{1, ..., q\} \\
		\id^{\tensor q} \tensor \eta(1) & i=q
	\end{cases}
	\quad \text{ and } \quad 
	\mathscr{D}(s^i) = \id^{\tensor i}\tensor \varepsilon\tensor \id^{q-i-1}\]
	where by a slight abuse of notation, $\eta(1)$ is the constant map at $x$.
	\begin{proof}
		Naturality of Alexander--Whitney implies that the following diagram commutes:
			\[\begin{tikzcd}[sep=large]
				...
				\arrow["{C_\bullet(\iota_x\times\id_{X^q})}", from=1-2, to=1-3]
				\arrow["{\AW_{*, X^q}}"', from=1-2, to=2-2]
				\arrow["{\AW_{q+1}}", from=1-3, to=2-3]
				\arrow["\cong", from=2-1, to=2-2]
				\arrow["{\mathscr{D}(d^0)}"', curve={height=30pt}, from=2-1, to=2-3]
				\arrow["{C_\bullet(\iota_x)\tensor \id^{\tensor q}}", from=2-2, to=2-3]
			\end{tikzcd}\]
		where $\iota_x$ is the inclusion at the basepoint. But now we make the identifications 
		\[C_\bullet(*)\tensor C_\bullet(X)^{\tensor q}\cong C_\bullet(X)^{\tensor q}, \text{ and } C_\bullet(\iota_x) = \eta(1)\]
		to conclude 
		\[ (\eta(1)\tensor \id^{\tensor q}) \circ \AW_q = \AW_{q+1} \circ  \hspace{2pt}C_\bullet(d^0)\]
		which implies $\mathscr{D}(d^0) = \eta(1)\tensor \id^{\tensor q}$. The arguments for the other maps are analogous.
	\end{proof}
\end{prop}

We now want to replace each column of $\mathscr{D}$:
\[\mathscr{D}^\bullet_p: C_p(*)\to C_p(X)\to C_p(X)^{\tensor 2}\to \cdots\] 
with its normalized cochain complex, which is the reason we computed these maps. Here we use an alternative formulation (prove) of the normalized cochain complex to obtain 
\begin{equation}
	\begin{split}
		N\mathscr{D}_p^q &= \bigcap_{i=0}^{q-1}\ker(s^i: \mathscr{D}_p^q\to \mathscr{D}_p^{q-1}) \\
		&= \bigcap_{i=0}^{q-1}\ker(\id^i\tensor \varepsilon\tensor \id^{\tensor q-i-1}) \\
		&=  (\ker(\varepsilon)^{\tensor q})_p \\
		&= \bigoplus_{m_1 + \cdots + m_q = p} \overline{C_{n_1}(X)} \tensor \cdots \tensor \overline{C_{n_q}(X)}
	\end{split}
\end{equation}
Now this is beginning to look very similar to the cobar construction on $C_\bullet(X)$. Because the vertical differential raises $q$, and the horizontal differential lowers $p$, when we totalize $N\mathscr{D}^\bullet_\bullet$ we take total degree $k = p-q$ to obtain a chain complex, similar to the construction of the cobar bicomplex.

\begin{thm}\label{thm:new_cobar}
    Suppose $x:= a=b$. Then 
    \begin{enumerate}
        \item There is an isomorphism $\tot N\mathscr{D} \to \Cobar(C_\bullet(X))$.
        \item There is a quasi-isomorphism $\Cobar(C_\bullet(X))\to \tot_\bullet(\mathscr{N})$.
    \end{enumerate}
    \begin{proof}
    We first show that (1) implies (2). Consider the following diagram where the solid arrows are quasi-isomorphisms previously constructed in this section, since totalization preserves quasi-isomorphisms. Their composition with (1) gives the desired quasi-isomorphism.
		\[\begin{tikzcd}
			% https://q.uiver.app/#q=
			& {\tot \mathscr{C}} & {\tot \mathscr{N}} \\
			{\tot N\mathscr{D}} & {\tot\mathscr{D}} \\
			{\Cobar(C_\bullet(X))}
			\arrow[two heads, from=1-2, to=1-3]
			\arrow["\AW"', shift right, from=1-2, to=2-2]
			\arrow[hook, from=2-1, to=2-2]
			\arrow["\EZ"', shift right, from=2-2, to=1-2]
			\arrow[curve={height=30pt}, dashed, from=3-1, to=1-3]
			\arrow["{(1)}", dashed, from=3-1, to=2-1]
		\end{tikzcd}\]
    To see (1), notice that $N\mathscr{D}$ is exactly the cobar bicomplex of $C_\bullet(X)$ in Definition \ref{def:cobar_bicomplex}, so that Proposition \ref{prop:cobar_bicomplex} yields an isomorphism $\tot N\mathscr{D}\to \Cobar(C_\bullet(X))$. 
    \end{proof}
\end{thm}
\begin{rem}
We have not yet used Adams' theorem. But what we have proven should suggest that the theorem is true for the following reason. We have exhibited a quasi-isomorphism between $\Cobar(C_\bullet(X))$ and the totalization of $\mathscr{C}$, which is itself the bicomplex constructed by taking singular chains on the cosimplicial loop space! So if we can somehow commute ``chains on a totalization'' and ``totalization of chains'' then we would obtain Adams' theorem. See the following diagram:
\[\begin{tikzcd}[cramped]
	...
	\arrow[shift left, no head, from=2-1, to=2-3]
	\arrow[shift left, no head, from=2-3, to=2-1]
\end{tikzcd}\]
The arrow on the right turns out to be an isomorphism, but it is beyond the scope of this thesis. 
\end{rem}

% Now we have all the ingredients. It's time to cook.
\begin{proof}[Proof of Theorem \ref{thm:looijenga}]
    In the quasi-isomorphism $\Cobar(C_\bullet(X))\to \tot_\bullet(\mathscr{N})$ produced in Proposition \ref{thm:new_cobar}, the truncation of $\mathscr{N}$ by $q$ corresponds to the tensor length truncation of $\Cobar$. It follows that the restriction
    \[\Cobar_{\leq n}(C_\bullet(X))\to \tot_\bullet(\mathscr{N}^{\leq n})\]
    is still a quasi-isomorphism. On zeroth homology, we therefore have an isomorphism when
    \[H_0\Cobar_{\leq n}(C_\bullet(X))\to H_0\tot_\bullet(\mathscr{N}^{\leq n})\]
    which by Corollary \ref{cor:gadish} yields an isomorphism when $x:= a=b$:
    \[\Z\pi_1(X,x)/ \mathcal{I}^{n+1} \to H_0\tot_\bullet(\mathscr{N}^{\leq n}).\]
    By Corollary \ref{cor:spectral_convergence}, we get a surjection
    \[\Z\pi_1(X,x)/ \mathcal{I}^{n+1} \to H_0\tot_\bullet(\mathscr{N}^{\leq n})\twoheadrightarrow H_n(X^n, X(n)^x_x)\]
    whose kernel is $\Z$, completing the proof.
\end{proof}